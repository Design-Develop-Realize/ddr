...
<table width='100%' border='0' cellspacing="0" cellpadding="5" bgcolor="#F7F7F7" bordercolor="#000080">
     <tr>
     <td bgcolor="#000080" align="center" width="20">&nbsp;</td>
...
<?     if (strtoupper($rsUser->Fields.$Item["U_ID"].$Value)!=strtoupper($rsAds["POSTER"].$Value))
    {
?>
	<?       if (($rsAds["Strength"]>=$rsGuestbook0["Strength"]*5 && $rsAds["Strength"]<=$rsGuestbook0["Strength"]*2) || ($rsAds["Strength"]>=30000 && $rsGuestbook0["Strength"]>=30000))
      {
?>
	<a href="declarewar_form.asp?Nation_ID=<?         echo $rsAds["Nation_ID"]; ?>&bynation=<?         echo ($rsGuestbook0["Nation_ID"].$Value); ?>">
	<img src="images/Swords.gif" title="Nation is within your strength range. You may declare war." border="0"></a> 
	<?       } ?>
<?     } ?>
</td>
...
<td align="left">
	<?     echo $rsAds["Last_Tax_Collection"]; ?>
	<?     if ($DateDiff["n"][$rsAds["Last_Tax_Collection"]][strftime("%m/%d/%Y %H:%M:%S %p")()]<60)
    {
?>
