...
if (${"search"}!="")
{
  ...
  if ($_GET["extended"]!=1)
  {
    ...
    $tfm_databaseFields=$Split["Declaring_Nation,Receiving_Nation,War_Declared_By_User,War_Declared_On_User"][","];
  }
    else
  {

    $tfm_exact="true";
    $tfm_databaseFields=$Split["Declaring_Nation_ID,Receiving_Nation_ID"][","];
  } 
...
echo $MM_conn_STRING;
if ($_GET["extended"]!=1 || is_numeric($_GET["search"])==false)
{

    echo "SELECT *  FROM War "+$tfm_SQLstr+" Order By War_Declaration_Date Desc";
}
  else
{

  $searchfor=str_replace("'","''",intval($_GET["search"]));
  $searchfor=htmlspecialchars($searchfor);
    echo "SELECT * FROM War where Declaring_Nation_ID = '".$searchfor."' OR Receiving_Nation_ID = '".$searchfor."' Order By War_Declaration_Date Desc";
} 
...
<? if (is_numeric($_GET["extended"])==true && is_numeric($_GET["search"])==true)
{
?>                                        
<link href="assets/tabcss.css" rel="stylesheet" type="text/css">
<div class="shadetabs">
<ul>
<li><a STYLE="text-decoration:none" href="nation_drill_display.asp?Nation_ID=<?   echo $_GET["search"]; ?>">Standard Display</a></li>
<li><a STYLE="text-decoration:none" href="nation_drill_display.asp?Nation_ID=<?   echo $_GET["search"]; ?>&Extended=1">Extended Display</a></li>
<li class="selected"><a STYLE="text-decoration:none" href="search_wars.asp?search=<?   echo $_GET["search"]; ?>&Extended=1">Search Wars</a></li>
<li><a STYLE="text-decoration:none" href="search_aid.asp?search=<?   echo $_GET["search"]; ?>&Extended=1">Search Aid</a></li>
</ul>
</div>
...
($rsAds==0)$Or$Not($rsAds_BOF==1)$Then; ?>
											...
											<!--#include file="search_wars_code.php" -->
...
//--></script><!--webbot BOT="GeneratedScript" endspan --><form action="search_wars.php" method="get" name="FrontPage_Form1" id="dirsearch" onsubmit="return FrontPage_Form1_Validator(this)" language="JavaScript">
<table border="1" width="100%" id="table1" cellspacing="0" cellpadding="5" bordercolor="#000080">
	<tr>
		<td width="50%">
		<p align="right">Search For Wars:</td>
