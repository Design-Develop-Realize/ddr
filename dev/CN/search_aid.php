...
if (${"search"}!="")
{
  ...
  $tfm_databaseFields=$Split["Declaring_Nation,Receiving_Nation,Resource_Sent_1,Resource_Sent_2,Resource_Sent_3,Aid_Issued_On_User,Aid_Issued_By_User"][","];
  $bellChar=chr(7);
...
if ($_GET["extended"]!=1 || is_numeric($_GET["search"])==false)
{
    ...
    echo "SELECT *  FROM Aid "+$tfm_SQLstr+" Order By Aid_Declaration_Date Desc";
}
  else
{
  ...
  $searchfor=htmlspecialchars($searchfor);
    echo "SELECT * FROM Aid where Declaring_Nation_ID = '".$searchfor."' OR Receiving_Nation_ID = '".$searchfor."' Order By Aid_Declaration_Date Desc";
} 
...
<li><a STYLE="text-decoration:none" href="search_wars.asp?search=<?   echo $_GET["search"]; ?>&Extended=1">Search Wars</a></li>
<li class="selected"><a STYLE="text-decoration:none" href="search_aid.asp?search=<?   echo $_GET["search"]; ?>&Extended=1">Search Aid</a></li>
</ul>
</div>
...
($rsAds==0)$Or$Not($rsAds_BOF==1)$Then; ?>
											...
											<!--#include file="search_aid_code.php" -->
...
//--></script><!--webbot BOT="GeneratedScript" endspan --><form action="search_aid.php" method="get" name="FrontPage_Form1" id="dirsearch" onsubmit="return FrontPage_Form1_Validator(this)" language="JavaScript">
<table border="1" width="100%" id="table1" cellspacing="0" cellpadding="5" bordercolor="#000080">
	<tr>
		<td width="50%">
		<p align="right">Search For Foreign Aid:</td>
